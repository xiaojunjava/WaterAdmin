...
<head>
    ...
    <script type="text/javascript">
        ...
        function loadTable(queryData) {
            ...
            //--获取数据库中的数据
            var jsonData = {
                url: '<%=request.getContextPath()%>/shipmaster/getShipmasterData?asaType=3',
                title: '运沙船主列表',
                halign: 'center',
                align: 'center',
                method: 'get',
            ...
            $('#list_asalyun').datagrid(jsonData);//加载数据
            $('#list_asalyun').datagrid('hideColumn','asaId');

        }
        function add() {
            $('#asalyunModuleForm').form('clear');//清除表单数据
            $("#tag").val("add");//操作标识
            $("#asalyunModule").panel({title: "&nbsp;添加运沙船主"});
            $('#asalyunModule').window('open');
        }

        function edit() {
            var rows = $('#list_asalyun').datagrid('getSelections');

            if (rows && rows.length == 1) {
                $("#tag").val("edit");//操作标识
                $('#asalyunModuleForm').form('clear');
            ...
            //先选择一条记录
            $("#asalyunModule").panel({title: "&nbsp;编辑运沙船主通讯录"});
            $('#asalyunModule').window('open');
        }
        function del() {
            var rows = $('#list_asalyun').datagrid('getSelections');
...
<div class="easyui-panel" title="查询" style="width:90%;padding:10px 10px 10px 10px;margin:0px 0px 1px 0px">
    <div style="margin-bottom:0px">
        <form id="asalyunSearchForm" method="post">
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ...
            <div style="display:inline; float:right">
                <a href="javascript:;" class="easyui-linkbutton" onclick="submitAsalyunSearchForm()" style="width:80px">查询</a>&nbsp;&nbsp;
                <a href="javascript:;" class="easyui-linkbutton" onclick="clearAsalyunSearchForm()" style="width:80px">清空</a>&nbsp;&nbsp;
            </div>
        </form>
    </div>
</div>
...
<!-- 运沙船主通讯录列表 -->
<div id="list_asalyun"></div>

<!-- 新增或编辑的弹出窗口 -->
<div id="asalyunModule" class="easyui-window" data-options="modal:true,closed:true,iconCls:'icon-save'"
    ...
    <div style="width:80%;max-width:300px;padding:10px 60px;">
        ...
        <form id="asalyunModuleForm" method="post">
...
<script>
    //查询
    function submitAsalyunSearchForm() {
        if ($('#asalyunSearchForm').form("validate")) {//通过校验
            var search_data = $('#asalyunSearchForm').serializeObject();
            loadTable(search_data);
        }
    }
    function clearAsalyunSearchForm(){
        $('#asalyunSearchForm').form('clear');
    }

    //新增&编辑
    function submitForm2() {
        if ($('#asalyunModuleForm').form("validate")) {//通过校验
            var tag = $('#tag').val();
            var actionUrl;
            var form_data = $('#asalyunModuleForm').serializeObject();
            if (tag && tag == 'add') {
                actionUrl = "<%=request.getContextPath()%>/shipmaster/saveShipmaster?asaType=3";
            } else if (tag && tag == 'edit') {
                actionUrl = "<%=request.getContextPath()%>/shipmaster/updateShipmaster?asaType=3&asaId="+$("#asaId").val();
            }
            $.ajax({
                type: "POST",
                url: actionUrl,
