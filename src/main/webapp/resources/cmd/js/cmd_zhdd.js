    ...
    $.ajax({
        ...
        success: function (data)
        {
            ...
            for(var i=0;i<data.length;i++){
                ...
                ls_str+='<div class="message" title="发送短信" onclick="sendMsg('+data[i].saId+',\''+data[i].saName+'\'),event.cancelBubble= true"></div>';
                ls_str+='<div class="more" title="查看详情" onclick="showOrHide(this),event.cancelBubble= true"></div>';
                ls_str+='</div>';
